...
import { Flower2, Clock, CheckCircle, XCircle } from 'lucide-react';
      ...
      <div className="space-y-3 flex-1">
        {pendingRequests.length > 0 ? (
          pendingRequests.map((req) => (
            <div
              key={req.id}
              className={`p-3 rounded-2xl border shadow-sm flex items-center gap-3 transition-transform hover:translate-x-1 ${
          ...
          <div
            className={`p-6 text-center border-2 border-dashed rounded-3xl h-full flex flex-col justify-center items-center ${
              darkMode ? 'border-slate-700 text-slate-400' : 'border-slate-200 text-slate-400'
            }`}
          >
