...
const GroupsView: React.FC<GroupsViewProps> = ({ groups = [], userSettings, contacts, onAdd, onEdit, onGroupClick, onDelete, onOpenBusinessConfig, venues = [] }) => {
  ...
  // --- RESTAURATION LOGIQUE : Notifications & Alertes ---
  ...
  const handleEmailAction = (contactId: any, subject: string, body: string) => {
    ...
    if (contact?.email) {
      ...
      alert("Pas d'email pour ce contact.");
    }
  };
    ...
    <div className="px-6 py-6 space-y-8 animate-in fade-in pb-32">
      <div className="flex justify-between items-center">
        <div className="flex items-center gap-3">
          ...
          <button onClick={onOpenBusinessConfig} className="p-2 rounded-xl text-slate-400 hover:text-indigo-600">
            <Cog size={20} />
          </button>
        </div>
        <div className="flex gap-2">
          <button onClick={() => setShowVenueCalendar(true)} className="px-4 py-2 rounded-xl text-xs font-black uppercase border-2 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-300 flex items-center gap-2 hover:bg-slate-50">
            <MapPin size={14} /> Calendrier Lieux
          </button>
          <button onClick={onAdd} className={`p-2 rounded-xl text-white bg-${themeColor}-600 shadow-lg active:scale-95 transition-all`}>
            <Plus size={20} />
          </button>
        </div>
      </div>
            ...
            <div key={group.id} onClick={() => onGroupClick(group)} className={`p-5 rounded-3xl border shadow-sm relative overflow-hidden cursor-pointer transition-all hover:-translate-y-1 ${userSettings.darkMode ? 'bg-slate-800 border-slate-700' : 'bg-white border-slate-100'}`}>
              ...
              {alerts.length > 0 ? (
                ...
                <div className="flex justify-between items-center mt-2">
                  <div className="flex gap-2">
                    {/* RESTAURATION DES ICÔNES D'OPTIONS AVEC PROTECTION ANTI-CRASH */}
