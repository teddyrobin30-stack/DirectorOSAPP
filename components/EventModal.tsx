...
const EventModal: React.FC<EventModalProps> = ({ isOpen, onClose, contacts, onSave, onDelete, userSettings, editEvent, onTriggerCommunication }) => {
  ...
  useEffect(() => {
    if (contactId && !isSmsEditing && isOpen) {
      ...
      setSmsMessage(`Bonjour ${contact?.name || ''}, je vous confirme notre RDV le ${dateStr} à ${startTime} (Durée: ${duration}). Objet: ${title || 'RDV'}${videoLink ? `\nLien Visio: ${videoLink}` : ''}`);
    }
          ...
          <div className="px-2 space-y-3">
             <div className="flex justify-between items-center">
               ...
               <div className="flex gap-2">
                 ...
                 }} className="px-3 py-1.5 rounded-lg bg-emerald-500 text-white text-[9px] font-black uppercase flex items-center gap-1"><ClipboardPaste size={12} /> Récupérer</button>
