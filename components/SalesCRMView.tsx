...
// ✅ Helper: safe ISO for date input "YYYY-MM-DD" (avoids Invalid Date / timezone surprises)
...
const SalesCRMView: React.FC<SalesCRMViewProps> = ({
  ...
  const handleSaveInbox = () => {
    ...
    // ✅ Correction: id + type_doc (si ta persistance filtre par type_doc)
    ...
    if (onUpdateClients && clients) {
      // ✅ Correction: éviter match sur email vide ('' === '')
  ...
  const handleValidateRequest = (item: InboxItem) => {
    ...
    if (onUpdateInbox) {
      // ✅ Correction: conserver type_doc si présent / l’ajouter si absent
      const updatedInbox = inbox.map(i =>
        i.id === item.id ? { ...i, type_doc: 'inbox' as const, status: 'processed' as const } : i
      );
  ...
  const handleArchiveRequest = (id: string) => {
    if (onUpdateInbox) {
      ...
      // ✅ Correction: conserver type_doc si présent / l’ajouter si absent
      const updatedInbox = inbox.map(i =>
        i.id === id ? { ...i, type_doc: 'inbox' as const, status: 'archived' as const } : i
      );
      onUpdateInbox(updatedInbox);
  ...
  const handleUpdateLastFollowUp = (id: string, date: string) => {
    if (!date || !onUpdateInbox) return;
    // ✅ Correction: conserver type_doc si présent / l’ajouter si absent
    onUpdateInbox(inbox.map(i => i.id === id ? { ...i, type_doc: 'inbox' as const, lastFollowUp: date } : i));
  };
                      ...
                      return (
                        ...
                        >
                          <div className="flex justify-between items-start mb-2">
                            ...
                            <span className={`text-[8px] font-black uppercase px-2 py-0.5 rounded ${lead.status === 'valide' ? 'bg-emerald-100 text-emerald-700' :
                            ...
                            <div className="space-y-1">
                              {daysLeads.map(l => (
                                <div
                                  ...
                                  className={`text-[8px] font-bold px-1 py-0.5 rounded truncate cursor-pointer hover:opacity-80 transition-opacity text-white ${l.status === 'valide' ? 'bg-emerald-500' :
                    ...
                    <div key={item.id} className={`p-4 rounded-2xl border flex flex-col md:flex-row justify-between md:items-center bg-white dark:bg-slate-800 ${isAlert ? 'border-orange-300 bg-orange-50 dark:bg-orange-900/10' : 'border-slate-100 dark:border-slate-700'}`}>
                      ...
                      <div className="flex items-center gap-4 md:gap-6 justify-between md:justify-end w-full md:w-auto">
                        ...
                        <button
                          ...
                          className={`flex items-center gap-2 px-3 py-1.5 rounded-lg border transition-all ${item.quoteSent
                            ? 'bg-emerald-50 border-emerald-200 text-emerald-700'
                            : 'bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 text-slate-400 hover:border-slate-300'
                            }`}
                        >
                          <div className={`w-3 h-3 rounded flex items-center justify-center border transition-colors ${item.quoteSent ? 'bg-emerald-500 border-emerald-500' : 'bg-transparent border-slate-300'
                            }`}>
                    ...
                    {sortedArchives.map(item => (
                      <tr key={item.id} className="hover:bg-slate-50 dark:hover:bg-slate-700/30 transition-colors">
                        ...
                        <td className="p-4 text-center">
                          <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${item.status === 'processed' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-500'
                            }`}>
                            {item.status === 'processed' ? 'Validé' : 'Non Abouti'}
                          </span>
                        </td>
                    ...
                    <div className="space-y-3">
                      {clientHistory.map(history => (
                        <div key={history.id} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 dark:bg-slate-900 border border-slate-100 dark:border-slate-800">
                          ...
                          <span className={`text-[9px] font-black uppercase px-2 py-1 rounded ${history.status === 'valide' ? 'bg-emerald-100 text-emerald-600' :
